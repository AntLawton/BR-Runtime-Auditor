...
/** Mock fetch simulating Firebase emulator responses for Tranche A probes. */
  ...
  return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (url.includes('/igv/engagements')) {
      ...
      if (auth.includes('invalid')) {
        return new Response('IGV facilitator access required', { status: 403 });
      }
    ...
    if (url.includes('/documents/igv-engagements/')) {
